...
class CoverageStore:
    ...
    def pending(self, grid: Iterable[Block]) -> list[Block]:
        ...
        does one that failed: only `hecho` takes a block out of the queue.
    ...
    def counts(self) -> Counts:
        ...
        return Counts(done=tally[DONE], failed=tally[FAILED], open=tally[OPEN])
    ...
    def close_block(self, block: Block, *, rows: int, requests: int) -> None:
        self._settle(block, DONE, rows=rows, requests=requests, error=None)
