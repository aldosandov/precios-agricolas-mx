...
from scraper.spiders.backfill import BackfillSpider, plan_session
...
# --- the sweep is over before the load starts (ALD-57) ---


def test_a_swept_block_is_not_covered_until_it_is_loaded(store, quarter):
    """The load runs after the sweep, so between them the NDJSON on disk is the
    only copy. A block that only got as far as disk stays in the queue."""
    spider = _spider(store, quarter)
    for request in _started(spider):
        _parse(spider, request, "range_destination_fixed")

    assert store.counts().done == 2
    assert store.pending(quarter) == quarter

    store.mark_loaded(*Q3_2011)

    assert store.pending(quarter) == []


def test_losing_the_ndjson_before_the_load_only_costs_requests(store, quarter):
    """Killing the session between sweep and load and deleting out/raw: the
    blocks come back and are fetched again, which is the whole point of not
    retiring them on `hecho` alone."""
    spider = _spider(store, quarter)
    for request in _started(spider):
        _parse(spider, request, "range_destination_fixed")

    assert _spider(store, quarter).blocks == quarter


def test_the_session_announces_the_disk_it_is_about_to_use(store, quarter):
    """Nothing is loaded until the sweep ends, so `--limit` is the disk control
    of the session: ~2.6 MB per block, measured."""
    session = plan_session(store, grid=quarter)

    line = session.announce()

    assert "2 bloques pendientes" in line
    assert "5 MB" in line


def test_the_announcement_counts_only_what_this_session_will_do(store):
    session = plan_session(store, grid=plan_grid(today=TODAY), limit=800)

    assert "esta sesión hará 800" in session.announce()
    assert "2.0 GB" in session.announce()


def test_the_spider_does_not_know_about_the_warehouse():
    """The coupling is what the three bugs of ALD-55 came out of. The only
    place the two halves meet is `main()`."""
    source = Path(backfill.__file__).read_text(encoding="utf-8")
    spider_half, main_half = source.split("def load_and_purge", 1)

    assert "transform" not in spider_half
    assert "transform.load" in main_half
